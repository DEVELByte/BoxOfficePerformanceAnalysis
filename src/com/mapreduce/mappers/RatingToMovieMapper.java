...
public class RatingToMovieMapper extends Mapper<Object, Text, Text, Text> {
			...
			if (p.getName().equals("movierating.txt")) {
				...
				while (line != null) {
					...
					if (movieMap.get(id) == null) {
						...
						totRating = Integer.parseInt(movieMap.get(id)
								.split("-")[1]) + Integer.parseInt(rating);
						movieMap.remove(id);
	...
	public void map(Object Key, Text Value, Context context)
		...
		if (movieMap.get(id) != null) {
			context.write(new Text("Movie"), new Text(id + "-" + movieDetail
					+ "-" + movieMap.get(id)));
		} else {
			context.write(new Text("Movie"), new Text(id + "-" + movieDetail
					+ "-" + 0 + "-" + 0));
